...
def pickUp():
  ...
  if inventory[currentTile] < MAXTILES:
    ...
    if not isInLake(playerX, playerY):
...
def isInLake(x, y):
  ...
  waterCount = 0
  for lakeY in range(y - 1, y + 2):
    for lakeX in range(x - 1, x + 2):
      if not (x < 0 or y < 0 or x > len(world) or y > len(world[0])) \
        and world[lakeX][lakeY] == WATER:
        waterCount += 1
...
def drawResource(x, y):
  ...
  if drawing == False:
    ...
    rendererT.goto( (x * TILESIZE) + 20, height - (y * TILESIZE) - 20 )
